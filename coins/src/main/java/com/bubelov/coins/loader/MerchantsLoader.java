...
public class MerchantsLoader extends SimpleCursorLoader {
    ...
    private static final String SELECT_MERCHANTS_QUERY = "select distinct m._id, m.latitude, m.longitude, m.amenity from merchants as m join currencies_merchants as mc on m._id = mc.merchant_id join currencies c on c._id = mc.currency_id where (latitude between ? and ?) and (m.longitude between ? and ?) and c.show_on_map = 1";

    private LatLngBounds bounds;

    private String amenity;

    public static Bundle prepareArguments(LatLngBounds bounds, Amenity amenity) {
        Bundle arguments = new Bundle();
        arguments.putParcelable(BOUNDS_KEY, bounds);
        arguments.putSerializable(AMENITY_KEY, amenity);
        return arguments;
    }
    ...
    public Cursor loadInBackground() {
        ...
        if (amenity == null) {
            return db.rawQuery(SELECT_MERCHANTS_QUERY,
            ...
            return db.rawQuery(SELECT_MERCHANTS_QUERY + " and m.amenity = ?",
